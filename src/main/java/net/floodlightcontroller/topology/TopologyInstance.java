...
//            }
            ...
            //tim duong di giua cac host (chi nhung switch ket noi truc tiep vs host)
//            for (DatapathId src : srcSws) { /* permute all member switches */
...
//                    log.info("Adding paths {}", paths);
    ...
    private List<Path> larac(DatapathId src, DatapathId dst,
        ...
        Path pc = buildPath(new PathId(src, dst), bt); /* guaranteed to be in same tree */
        
        if (pc != null && !pc.getPath().isEmpty()) { /* should never be null, but might be empty */
            setPathCosts(pc);
            
            if (pc.getLatency().getValue() <= deltaDelay) {
            	A.add(pc);
            	log.debug("Found shortest path in LARAC {}", pc);
            	//log.info("Found shortest path with Costs in LARAC {}", pc);
            ...
            bt = dijkstra(copyOfLinkDpidMap, dst, delay, true);
            aSrc.setBroadcastTree(bt);
            
            Path pd = buildPath(new PathId(src, dst), bt);
            
            if (pd != null && !pd.getPath().isEmpty()) {
            	setPathCosts(pd);
            	
                if (pd.getLatency().getValue() > deltaDelay) {
                    log.debug("There is no solution for {} to {} !", src, dst);
                    log.info("There is no solution for {} to {} !", src, dst);
                    
                    return A;
                } else {
                    double lambda = 0;
                    
                    Map<Link, Integer> c_lambda = new HashMap<Link, Integer>();
                    Path r;
                    ...
                    while(true) {
                        ...
                        lambda = (getPathCosts(pc) - getPathCosts(pd)) / (pc.getLatency().getValue() - pd.getLatency().getValue());
    //                  
                        /*--- compute c_lambda 
                         * c_lambda = cost + lambda*delay;
                         * ---*/
                        for (NodePortTuple npt : links.keySet()) {
                            if (links.get(npt) == null) {
                                continue;
                            ...
                            for (Link link : links.get(npt)) {
                                ...
                                c_lambda.put(link, (int)(cost.get(link) + Math.abs(lambda)*delay.get(link)));
                            }
                        }
                        /* end compute c_lambda  */
                        
//                        log.info("-----------=============lap??? lambda: {}", lambda);
                        
                        bt = dijkstra(copyOfLinkDpidMap, dst, c_lambda, true);
                        aSrc.setBroadcastTree(bt);
                        r = buildPath(new PathId(src, dst), bt);
                        
                        if (r != null && !r.getPath().isEmpty()) {
                            setPathCosts(r);
                            
//                            if (getPathCosts(r) == getPathCosts(pc)) {
//                            log.info("-----------=============lap??? cost r: {}, cost pc: {}", getPathCosts(r), getPathCosts(pc));
                            if (Math.abs(getPathCosts(r) - getPathCosts(pc)) <= 100){
                                setPathCosts(pd);
                                A.add(pd);
                                ...
                                if (r.getLatency().getValue() <= deltaDelay) {
                                    pd = r;
                                } else {
                                    pc = r;
                                }
...
//                            log.info("There is no solution for {} to {} with LAMBDA!", src, dst);
            ...
            log.debug("No paths found in LARAC!");
    ...
    private List<Path> considerEachMetric(DatapathId src, DatapathId dst, Long deltaDelay) {
        ...
        for(int i = 0; i < METRIC.length; i++) {
        	Map<Link, Integer> metric = calculateCost(METRIC[i]);
    ...
    public Integer getPathCosts(Path p) {
        ...
        for (int i = 0; i <= p.getPath().size() - 2; i = i + 2) {
            ...
            for (LinkCost ls: linkCostsRF) {
    			if (src.toString().equals(ls.getSrc()) && dst.toString().equals(ls.getDst())) {
    				log.debug("Maitching link found: {}", ls);
    				cost = cost + ls.getCosts();
    			} else if (src.toString().equals(ls.getDst()) && dst.toString().equals(ls.getSrc())) {
    				log.debug("Maitching link found: {}", ls);
    				cost = cost + ls.getCosts();
    			}
    		}
    ...
    public Map<Link,Integer> calculateCost(String nameMetric) {
        Map<Link, Integer> linkCost = new HashMap<Link, Integer>();    
        ...
        for (int i = 0; i <= p.getPath().size() - 2; i = i + 2) {
            ...
            for (Link l : links.get(p.getPath().get(i))) {
                if (l.getSrc().equals(src) && l.getDst().equals(dst) &&
                    ...
                    log.debug("Maitching link found: {}", l);
...
//        log.info("Total cost is {} of path {}", cost.toString(), p.toString());
