...
public class TopologyManager implements IFloodlightModule, ITopologyService, ITopologyManagerBackend,
	...
	protected static volatile PATH_METRIC pathMetric = PATH_METRIC.HOPCOUNT_AVOID_TUNNELS; // default:
																							// compute
																							// paths
																							// on
																							// hop
	...
	// ***** Viet them Khoi tao deltaDelay and Src, Dst
	protected static volatile Long deltaDelay;
	protected static volatile ArrayList<String> listSwitch = new ArrayList<String>();

	protected static Long getDeltaDelay() {
		return deltaDelay;
	}

	public void getListSwitch(String src, String dst) {
		listHostsRF = ReadFile.getListHostsFromFile();

		listSwitch.clear();

		for (ListHost lhostSrc : listHostsRF) {
			if (lhostSrc.getHost().equals(src)) {
				listSwitch.add(lhostSrc.getSwitch());
			}
			if (lhostSrc.getHost().equals(dst)) {
				listSwitch.add(lhostSrc.getSwitch());
			}
		}
		// return listSwitch;
	...
	protected static ArrayList<String> getListSwitch() {
		return listSwitch;
	}
	// *** END
		...
		public void run() {
			try {
				if (ldUpdates.peek() != null) { /*
												 * must check here, otherwise
												 * will run every interval
												 */
	...
	public void init(FloodlightModuleContext context) throws FloodlightModuleException {
		...
		String metric = configOptions.get("pathMetric") != null ? configOptions.get("pathMetric").trim().toLowerCase()
				: null;
	...
	protected Command dropFilter(DatapathId sw, OFPacketIn pi, FloodlightContext cntx) {
		...
		if (isNotBlocked(sw, inPort) == false) {
			if (log.isTraceEnabled()) {
				log.trace("Ignoring packet because of topology " + "restriction on switch={}, port={}", sw.getLong(),
	...
	public void doMultiActionPacketOut(byte[] packetData, IOFSwitch sw, Set<OFPort> ports, FloodlightContext cntx) {
		...
		if (log.isTraceEnabled()) {
			log.trace("write broadcast packet on switch-id={} " + "interaces={} packet-data={} packet-out={}",
	...
	public List<LDUpdate> applyUpdates() {
		...
		while (ldUpdates.peek() != null) {
			...
			case LINK_UPDATED:
				addOrUpdateLink(update.getSrc(), update.getSrcPort(), update.getDst(), update.getDstPort(),
	...
	protected void addOrUpdateSwitch(DatapathId sw) {
		 ...
		 * 
		 * addSwitch(sw); for (OFPortDesc p :
		 * switchService.getSwitch(sw).getPorts()) { addPortToSwitch(sw,
		 * p.getPortNo()); }
		 */
		return;
	}
	...
	protected boolean createNewInstance(String reason, boolean forced) {
		...
		// switchPorts contains only ports that are part of links. Calculation
		// of broadcast ports needs set of all ports.
		...
		TopologyInstance nt = new TopologyInstance(switchPorts, blockedPorts, openflowLinks, broadcastDomainPorts,
				tunnelPorts, switchPortLinks, allPorts, interClusterLinks);
	...
	protected Set<NodePortTuple> identifyBroadcastDomainPorts() {
		...
		if (additionalNpt.size() > 0) {
			log.warn("The following switch ports have multiple "
					+ "links incident on them, so these ports will be treated " + " as braodcast domain ports. {}",
					additionalNpt);
	...
	private void addLinkToStructure(Map<NodePortTuple, Set<Link>> s, Link l) {
		...
		/*
		 * Since we don't include latency in .equals(), we need to explicitly
		 * remove the existing link (if present). Otherwise, new latency values
		 * for existing links will never be accepted.
		 */
		s.get(n1).remove(l);
		s.get(n2).remove(l);
		s.get(n1).add(l);
	...
	/**
	 * Delete the given link from the data structure. Returns true if the link
	 * was deleted.
	 * 
	 * @param s
	 * @param l
	...
	public void removeLink(DatapathId srcId, OFPort srcPort, DatapathId dstId, OFPort dstPort) {
		Link link = new Link(srcId, srcPort, dstId, dstPort,
				U64.ZERO /*
							 * does not matter for remove (not included in
							 * .equals() of Link)
							 */);
		removeLink(link);
	}
